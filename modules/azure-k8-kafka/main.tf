...
/*
resource "kubernetes_service" "kafka-external" {
  metadata {
    annotations = { "external-dns.alpha.kubernetes.io/hostname" = "${kubernetes_stateful_set.kafka.metadata.0.labels.app}.cluster.local" }
    name      = "${var.kafka_name}-external"
    namespace = "${var.namespace}"
    labels    = { 
	   app = "${kubernetes_stateful_set.kafka.metadata.0.labels.app}" 
	   "app.kubernetes.io/component" = "kafka-broker"
	   "app.kubernetes.io/instance" = "${kubernetes_stateful_set.kafka.metadata.0.labels.app}"
  ...
  spec {
    ...
    selector = { 
	   ...
	   "app.kubernetes.io/instance" = "${kubernetes_stateful_set.kafka.metadata.0.labels.app}"
       "app.kubernetes.io/name" = "kafka"
       "statefulset.kubernetes.io/pod-name" = "${kubernetes_stateful_set.kafka.metadata.0.labels.app}-0"	   
	   }
	type     = "LoadBalancer"
	load_balancer_ip  = "${azurerm_public_ip.kafkaLBIP.ip_address}"
  }
}
*/
    ...
    template {
      ...
      spec {
        container {
          ...
          command = ["sh", "-exc", "unset KAFKA_PORT && \\\nexport KAFKA_BROKER_ID=$${POD_NAME##*-} && \\\nexport KAFKA_ADVERTISED_LISTENERS=PLAINTEXT://$${POD_IP}:9092,EXTERNAL://$${LB_IP}:41090 && \\\nexport KAFKA_LISTENER_SECURITY_PROTOCOL_MAP=PLAINTEXT:PLAINTEXT,EXTERNAL:PLAINTEXT && \\\nexec /etc/confluent/docker/run\n"]
		  #command = ["sh", "-exc", "unset KAFKA_PORT && \\\nexport KAFKA_BROKER_ID=$${POD_NAME##*-} && \\\nexport KAFKA_ADVERTISED_LISTENERS=PLAINTEXT://$${POD_IP}:9092 && \\\nexport KAFKA_ADVERTISED_LISTENERS=PLAINTEXT://$${LB_IP}:41090 && \\\nexec /etc/confluent/docker/run\n"]
		  #command = ["sh", "-exc", "unset KAFKA_PORT && \\\nexport KAFKA_BROKER_ID=$${POD_NAME##*-} && \\\nexport KAFKA_ADVERTISED_LISTENERS=PLAINTEXT://$${POD_IP}:9092 && \\\nexec /etc/confluent/docker/run\n"]
          port {
            name           = "kafka"
            container_port = 9092
		  ...
		  port {
            name           = "kafka-external"
            container_port = 41090
          }
		  port {
            name           = "kafka-ingress"
            container_port = 31627
          }
          env {
            name = "POD_IP"
            value_from {
		 ...
		 /* env {
            ...
            value_from {
              field_ref {
                #field_path = "status.podIP"
		        # field_path = "${kubernetes_service.kafka-external.status.ingress.ip}" 
		  		 #field_path = "${status.loadbalancer.ingress.ip}"
				 field_path = "azurerm_public_ip.kafkaLBIP.ip_address"
              }
            }
          }*/
          env {
		  ...
		  env {
            name  = "LB_IP"
            value = "${azurerm_public_ip.kafkaLBIP.ip_address}"
          }
		  env {
		  ...
		  env {
            ...
            value = "EXTERNAL://${azurerm_public_ip.kafkaLBIP.ip_address}:41090" 
          }		  
          env {
...
resource "azurerm_public_ip" "kafkaLBIP" {
  name                = "KafkaPublicIPForLB"
...
resource "azurerm_lb" "kafkaLB" {
  ...
  frontend_ip_configuration {
    ...
    public_ip_address_id = azurerm_public_ip.kafkaLBIP.id
  }
}

resource "azurerm_lb_rule" "kafkaLBRule" {
  resource_group_name            = var.resource_group_name
  loadbalancer_id                = azurerm_lb.kafkaLB.id
  name                           = "LBRule"
  protocol                       = "Tcp"
  frontend_port                  = 41090
  backend_port                   = 41090
  frontend_ip_configuration_name = "PublicIPAddress"
  probe_id                       = azurerm_lb_probe.kafkaprobe.id
}

resource "azurerm_lb_probe" "kafkaprobe" {
  resource_group_name = var.resource_group_name
  resource_group_name = var.resource_group_name
  loadbalancer_id     = azurerm_lb.kafkaLB.id
